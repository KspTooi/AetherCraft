...
✅ **BUG3修复记录**（2025-01-XX）
1. **动态权限判断**：
   - `saveModelVariantParam`接口移除@RequirePermissionRest注解
   - `removeModelVariantParam`接口移除@RequirePermissionRest注解
   - 在Controller中使用`AuthService.hasPermission`进行动态权限判断

2. **权限细化分类**：
   - 原权限`admin:model:variant:param:save`拆分为：
     - `admin:model:variant:param:save:global` - 新增/编辑全局模型变体参数
     - `admin:model:variant:param:save:self` - 新增/编辑个人模型变体参数
   - 原权限`admin:model:variant:param:delete`拆分为：
     - `admin:model:variant:param:remove:global` - 删除全局模型变体参数
     - `admin:model:variant:param:remove:self` - 删除个人模型变体参数

3. **权限枚举更新**：
   - 更新`PermissionEnum.java`中的权限定义
   - 根据dto.global参数值动态选择对应权限进行验证
   - `getModelVariantParamDetails`接口使用查看权限

✅ **权限枚举补充**（仅参数管理权限）
- `ADMIN_MODEL_VARIANT_PARAM_VIEW` - 查看模型变体参数
- `ADMIN_MODEL_VARIANT_PARAM_SAVE_GLOBAL` - 新增/编辑全局模型变体参数
- `ADMIN_MODEL_VARIANT_PARAM_SAVE_SELF` - 新增/编辑个人模型变体参数
- `ADMIN_MODEL_VARIANT_PARAM_REMOVE_GLOBAL` - 删除全局模型变体参数
- `ADMIN_MODEL_VARIANT_PARAM_REMOVE_SELF` - 删除个人模型变体参数
